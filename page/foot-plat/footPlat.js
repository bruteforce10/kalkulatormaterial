...
const calculateButtonFootPlat = document.querySelector("#hitungMaterial");
const form = document.querySelector("#formInput");
import inputValid from "../../JS/utilities/utilities.js";
const storageKeys = "FOOTPLAT_KEY";
import { generateMaterial } from "../../JS/rumus/material.js";
const tableFootPlat = document.querySelectorAll("td");
let inputValidations = false;
import { generateTulutama, generateTulSengkang } from "../../JS/rumus/besi.js";
const kebFootPlat = document.querySelectorAll("#kebutuhanMaterial");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  class PerhitunganFootPlat {
    ...
    volumePekerjaanFootPlat(output) {
  ...
  calculateButtonFootPlat.addEventListener("click", function (e) {
    e.preventDefault();
    validateInputs();

    const perhitunganFootPlat = new PerhitunganFootPlat(inputs);
    if (!inputValidations) {
      perhitunganFootPlat.volumePekerjaanFootPlat(kebFootPlat);
      perhitunganFootPlat.volumePekerjaanFootPlat(tableFootPlat);
    }
  });
});
  ...
  if (localStorage.getItem(storageKeys)) {
    ...
    if (kebFootPlat) {
      for (let x = 0; x < arry.length; x++) {
        kebFootPlat[x].innerHTML = `${arry[x]}`;
      }
      for (let x = 0; x < arry.length; x++) {
        tableFootPlat[x].innerHTML = `${arry[x]}`;
      }
    }
  } else {
    return;
