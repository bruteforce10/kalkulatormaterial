...
const calculateButtonKolomStruktur = document.querySelector("#hitungMaterial");
const form = document.querySelector("#formInput");
import inputValid from "../../JS/utilities/utilities.js";
const storageKeys = "KOLOMSTRUKTUR_KEY";
import { generateMaterial } from "../../JS/rumus/material.js";
const tableKolomStrukutur = document.querySelectorAll("td");
let inputValidation = false;
import { generateTulutama, generateTulSengkang } from "../../JS/rumus/besi.js";
const kebKolomStruktur = document.querySelectorAll("#kebutuhanMaterial");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  calculateButtonKolomStruktur.addEventListener("click", function (e) {
    e.preventDefault();
    validateInputs();
  ...
  if (localStorage.getItem(storageKeys)) {
    ...
    if (!kebKolomStruktur) {
      for (let x = 0; x < arry.length; x++) {
        kebKolomStruktur[x].innerHTML = `${arry[x]}`;
      }
      for (let x = 0; x < arry.length; x++) {
        tableKolomStrukutur[x].innerHTML = `${arry[x]}`;
      }
    }
  } else {
    return;
