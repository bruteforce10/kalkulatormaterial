...
const calculateButtonBalokStruktur = document.querySelector("#hitungMaterial");
const form = document.querySelector("#formInput");
import inputValid from "../../JS/utilities/utilities.js";
const storageKey = "BALOKSTRUKTUR_KEY";
import { generateMaterial } from "../../JS/rumus/material.js";
const tableBalokStrukutur = document.querySelectorAll("td");
let inputValidation = false;
import { generateTulutama, generateTulSengkang } from "../../JS/rumus/besi.js";
const kebBalokStruktur = document.querySelectorAll("#kebutuhanMaterial");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  class PerhitunganBalok {
    ...
    volumePekerjaanBalokStruktur(output) {
      // Volume Besi
      let volBesiUtama = (
  ...
  calculateButtonBalokStruktur.addEventListener("click", function (e) {
    e.preventDefault();
    validateInputs();

    const perhitunganBalok = new PerhitunganBalok(inputs);
    if (!inputValidation) {
      perhitunganBalok.volumePekerjaanBalokStruktur(kebBalokStruktur);
      perhitunganBalok.volumePekerjaanBalokStruktur(tableBalokStrukutur);
    }
  });
});
  ...
  if (localStorage.getItem(storageKey)) {
    ...
    if (kebBalokStruktur) {
      for (let x = 0; x < arry.length; x++) {
        kebBalokStruktur[x].innerHTML = `${arry[x]}`;
      }
      for (let x = 0; x < arry.length; x++) {
        tableBalokStrukutur[x].innerHTML = `${arry[x]}`;
      }
    }
  } else {
    return;
